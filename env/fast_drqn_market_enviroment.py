...
class MarketEnvironment(gym.Env):
    ...
    def __init__(self, data: pd.DataFrame, portfolio: Portfolio, initial_balance: float = 1000, render_mode = 'human', enable_logger: bool = True):
        ...
        setup_file_logger(__name__, f'{ROOT}/logs/fast_market_environment.log', log_level=logging.INFO, will_propogate=True)

        # Initialize attributes
        required_columns = {'Adjusted_Close', 'Volume'}
        if not required_columns.issubset(data.columns):
            self.market_env_logging.debug(f"{data.columns}")
            raise ValueError(f"DataFrame must contain the following columns: {required_columns}")

        self.data = data.reset_index(drop=True)
        # self.market_return = (self.data['Adjusted_Close'][-1] * self.initial_balance)/self.data['Adjusted_Close'][0] # This a metric for performance checking

        # self.market_return  = (self.data.iloc[-1][['Adjusted_Close']].values )/self.data.iloc[0][['Adjusted_Close']].values
        # self.market_return  = (self.data.iloc[-1][['Adjusted_Close']].values - self.data.iloc[0][['Adjusted_Close']].values )/self.data.iloc[0][['Adjusted_Close']].values
        # self.market_return *= 1000
        # self.market_return *= 1*self.data.iloc[0][['Adjusted_Close']].values

        initial_price = self.data.iloc[0]['Adjusted_Close']
        final_price = self.data.iloc[-1]['Adjusted_Close']
        # Calculate Market Return
        self.market_return = (final_price - initial_price) / initial_price * 100
        # Final value of one share, if you bought 1 share and held at time step = 0 
        # final_value = (initial_balance/initial_price) * (1 + self.market_return / 100)
   ...
   #@time_decorator
    def reset(self, seed=None, options=None):
        ...
        # Log reset state
        self.market_env_logging.info(f"Environment BEFORE RESET: Balance: {self.portfolio.balance}, Holdings: {self.portfolio.holdings}, Net Profit: {self.portfolio.net_profit}")
